...
public class StompGameClient2 : MonoBehaviour
{
    ...
    public RemotePlayerManager remotePlayerManager; // 인스펙터에서 할당

    private void HandleMessageFrame(string headers, string body)
    {
        try
        {
            var evt = JsonUtility.FromJson<EventWrapperDto>(body);
            if (evt == null || evt.payload == null || evt.payload.operation == null)
            {
                Debug.LogWarning("[STOMP] MESSAGE payload has no operation. body=" + body);
                return;
            }

            var op = evt.payload.operation;
            Debug.Log($"[STOMP] MESSAGE MOVE: playerId={op.playerId}, pos=({op.x},{op.y},{op.z})");

            if (remotePlayerManager != null)
            {
                remotePlayerManager.ApplyRemoteMove(op.playerId, op.x, op.y, op.z);
            }
        }
        catch (Exception e)
        {
